...
# 创建数据库链接
client = pymongo.MongoClient('192.168.1.80')
db = client['cnblogs']
collection = db['home']
...
def parse_data(response):
    ...
    for div in divs:
        _data = {
            # 文章标题
            'title':
            div.find('.titlelnk').text(),
            # 文章详情页链接
            'url':
            div.find('.titlelnk').attr('href'),
            # 文章简介
            'short':
            div.find('.post_item_summary').text(),
            # 博主
            'author':
            div.find('.post_item_foot .lightblue').text(),
            # 博主的主页链接
            'author_url':
            div.find('.post_item_foot .lightblue').attr('href'),
            # 发布时间
            'create_time':
            div.find('.post_item_foot').text().split()[2] + ' ' +
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(total), desc='抓取进度', ncols=100):
        if i == 0:
            ...
            page = i + 1
            url = 'https://www.cnblogs.com/sitehome/p/' + str(page)
            #main(url, i + 1)
            main(url)
        time.sleep(1)